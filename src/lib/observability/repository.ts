...
export function createObservabilityRepository(database: PrismaClient) {
  async function listRuns(filters: ExecutionFilters): Promise<ExecutionRunSummary[]> {
    ...
    if (!filters.kind || filters.kind === 'workflow') {
      const rows = await database.workflowRun.findMany({
        where: { workflowId: filters.workflowId, triggerType: filters.trigger, actorId: filters.actorId, createdAt },
    ...
    if (!filters.kind || filters.kind === 'script') {
      const rows = await database.build.findMany({ where: { scriptId: filters.scriptId, triggeredBy: filters.trigger, createdAt }, include: { script: true }, orderBy: { createdAt: 'desc' }, take: filters.limit })
    ...
    if (!filters.kind || filters.kind === 'remote') {
      const rows = await database.remoteExecution.findMany({ where: { triggeredBy: filters.trigger, requestedAt: createdAt, profile: filters.projectId ? { projectId: filters.projectId } : undefined }, orderBy: { requestedAt: 'desc' }, take: filters.limit })
  ...
  async function getDashboard(filters: ExecutionFilters): Promise<ExecutionDashboard> {
    ...
    const [enabledSchedules, disabledSchedules] = await Promise.all([
      database.workflowTrigger.count({ where: { type: 'cron', enabled: true } }),
      database.workflowTrigger.count({ where: { type: 'cron', enabled: false } }),
    ])
    return {
  ...
  async function getRunDetail(kind: ExecutionKind, id: string) {
    if (kind === 'workflow') {
      const run = await database.workflowRun.findUnique({ where: { id }, include: { workflow: true, nodeRuns: true } })
      if (!run) return null
    ...
    if (kind === 'script') {
      const build = await database.build.findUnique({ where: { id }, include: { script: { select: { id: true, name: true } } } })
    ...
    const remote = await database.remoteExecution.findUnique({ where: { id } })
