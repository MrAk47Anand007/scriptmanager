...
export function WorkflowSidebar() {
  const dispatch=useAppDispatch();const items=useAppSelector(selectWorkflows);const active=useAppSelector(selectActiveWorkflow);const [collapsed,setCollapsed]=useState(false)
  useEffect(()=>{void dispatch(fetchWorkflows())},[dispatch])
  const create=async(templateKey?:keyof typeof workflowTemplates)=>{const definition=templateKey?workflowTemplates[templateKey].definition:{schemaVersion:1 as const,name:'Untitled workflow',nodes:[],edges:[]};const item=await createWorkflowRuntime({name:definition.name,definition});dispatch(selectWorkflow(item));void dispatch(fetchWorkflows())}
