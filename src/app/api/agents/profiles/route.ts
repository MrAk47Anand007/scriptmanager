...
export async function GET(request: Request) { return NextResponse.json(await prisma.agentProfile.findMany({ where: { workspaceId: request.headers.get('x-scriptmanager-workspace-id') ?? new URL(request.url).searchParams.get('workspaceId') ?? 'default' }, include: { providerConfig: true }, orderBy: { createdAt: 'desc' } })) }
