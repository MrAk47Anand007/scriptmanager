...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const secretReference = secret ? await storeResourceSecret(prisma, { resourceType: 'workflow-trigger', resourceId: triggerId, field: 'webhook-signing', name: `workflow-trigger:${triggerId}:webhook-signing`, workspaceId: authorization.context.workspaceId }, secret, authorization.context.userId) : null
