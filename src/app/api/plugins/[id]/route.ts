...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = body.action === 'trust' ? await registry.trust(auth.context.workspaceId, id) : body.action === 'enable' ? await registry.enable(auth.context.workspaceId, id) : body.action === 'disable' ? await registry.disable(auth.context.workspaceId, id) : body.action === 'health' ? await registry.setHealth(auth.context.workspaceId, id, Boolean(body.healthy), body.message) : body.action === 'settings' ? await registry.updateSettings(auth.context.workspaceId, id, body.settings) : null
