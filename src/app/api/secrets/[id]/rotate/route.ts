...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(await defaultSecretVaultService().rotateSecret(id, body.plaintext, { actorType: 'user', actorId: actor.actorId, workspaceId: actor.workspaceId, capability: 'secret:write', resource: body.resource ?? '*', reason: body.reason ?? 'manual rotation' }))
  } catch (error) {
