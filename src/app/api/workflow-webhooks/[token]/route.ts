...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (!verifyWorkflowWebhookSignature(rawBody, request.headers.get('x-scriptmanager-signature'), decryptString(trigger.webhookSecretEncrypted))) return Response.json({ error: 'Invalid signature' }, { status: 401 })
