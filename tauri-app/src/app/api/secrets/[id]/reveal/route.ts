...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const authorization = await authorizeRequest(request, 'secret', 'reveal')
    if (authorization.response) return authorization.response
    const actor = requireTrustedContext(await resolveTrustedRequestContext(request, prisma))
    const { id } = await params
    const body = await request.json()
    return NextResponse.json(await defaultSecretVaultService().revealSecretOnce(id, { actorType: 'user', actorId: actor.actorId, workspaceId: actor.workspaceId, capability: 'secret:reveal', resource: body.resource ?? '*', reason: body.reason ?? 'manual reveal' }))
  } catch (error) {
